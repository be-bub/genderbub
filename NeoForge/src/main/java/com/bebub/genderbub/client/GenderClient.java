...
import com.bebub.genderbub.network.GenderPacket;
...
public class GenderClient {
    ...
    public static void init() {
        jadePresent = net.neoforged.fml.ModList.get().isLoaded("jade");
        jadeChecked = true;
    }
    ...
    public static void onEntityInteract(PlayerInteractEvent.EntityInteract event) {
        ...
        if (GenderConfig.isSyncConfigRules()) {
            ...
            PacketDistributor.sendToServer(new GenderPacket(entity.getId(), isOffhand));
            event.setCanceled(true);
            event.setCancellationResult(InteractionResult.SUCCESS);
        } else {
