...
public class EggLayingHandler {
    ...
    public static void onEntityJoin(EntityJoinLevelEvent event) {
        ...
        Mob nearestChicken = null;
        double nearestDist = 2.0;
        
        for (Mob mob : event.getLevel().getEntitiesOfClass(Mob.class, itemEntity.getBoundingBox().inflate(2.0))) {
            if (mob instanceof Chicken) {
                double dist = mob.distanceToSqr(itemEntity);
                if (dist < nearestDist) {
                    nearestDist = dist;
                    nearestChicken = mob;
                }
