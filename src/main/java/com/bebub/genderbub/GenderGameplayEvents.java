...
public class GenderGameplayEvents {
    ...
    private boolean canBreed(LivingEntity parentA, LivingEntity parentB) {
        ResourceLocation idA = ForgeRegistries.ENTITY_TYPES.getKey(parentA.getType());
        ResourceLocation idB = ForgeRegistries.ENTITY_TYPES.getKey(parentB.getType());
        String mobIdA = idA != null ? idA.toString() : "";
        String mobIdB = idB != null ? idB.toString() : "";
        
        if (GenderConfig.getGenderOnlyMobs().contains(mobIdA) || 
            GenderConfig.getGenderOnlyMobs().contains(mobIdB)) {
            return true;
        }
    ...
    public void onEntityJoinWorld(EntityJoinLevelEvent event) {
        ...
        boolean isIceFireDragon = entity.getClass().getName().contains("iceandfire.entity.EntityDragonBase");
        if (isIceFireDragon) {
        ...
        if (hasScanner) {
            ...
            if (isExternalMob) {
                ...
                if (entity instanceof AgeableMob) {
                    isBaby = ((AgeableMob) entity).isBaby();
                }
                ...
                if (currentGender != null && !currentGender.equals("none")) {
                    ...
                    if (isVillager || isZombieVillager) {
                        ...
                        if (sterile) {
                            if (isBaby) {
                                key = "genderbub.villager.sterile.baby";
                            } else {
                                key = currentGender.equals("male") ? "genderbub.villager.sterile.male" : "genderbub.villager.sterile.female";
                            }
                        } else {
                            if (isBaby) {
                                key = "genderbub.villager.baby";
                            } else {
                                key = currentGender.equals("male") ? "genderbub.villager.male" : "genderbub.villager.female";
                            }
                        }
                        ...
                        if (sterile) {
                            if (isBaby) {
                                key = "genderbub.gender.sterile.baby";
                            } else {
                                key = currentGender.equals("male") ? "genderbub.gender.sterile.male" : "genderbub.gender.sterile.female";
                            }
                        } else {
                            if (isBaby) {
                                key = "genderbub.gender.baby";
                            } else {
                                key = currentGender.equals("male") ? "genderbub.gender.male" : "genderbub.gender.female";
                            }
                        }
                        displayComponent = Component.translatable(key).withStyle(style -> style.withColor(color));
                    }
