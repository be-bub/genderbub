...
public class GenderConfig {
    ...
    public static class ConfigData {
        ...
        public ConfigData() {
            ...
            settings.genderOnlyMobs = new ArrayList<>();
            
            interactions.add(new InteractionRule("minecraft:cow", Arrays.asList("male", "sterile"), Arrays.asList("minecraft:bucket")));
            interactions.add(new InteractionRule("minecraft:goat", Arrays.asList("male", "sterile"), Arrays.asList("minecraft:bucket")));
            
            eggRules.add(new EggRule("minecraft:chicken", Arrays.asList("male", "sterile")));
        }
    }
    ...
    public static void queueScan() { scanQueued = true; }
        ...
        if (!configPath.toFile().exists()) {
            ...
            try (Reader reader = new FileReader(configPath.toFile())) {
                ...
                if (root.has("settings")) {
                    ...
                    if (settings.has("genderOnlyMobs")) {
                        data.settings.genderOnlyMobs = new ArrayList<>();
                        for (JsonElement e : settings.getAsJsonArray("genderOnlyMobs")) {
                            data.settings.genderOnlyMobs.add(e.getAsString());
                        }
                    }
                }
                ...
                if (root.has("interactions")) {
                    data.interactions = GSON.fromJson(root.get("interactions"), new com.google.gson.reflect.TypeToken<List<InteractionRule>>(){}.getType());
                    if (data.interactions == null) data.interactions = new ArrayList<>();
                }
                
                if (root.has("eggRules")) {
                    data.eggRules = GSON.fromJson(root.get("eggRules"), new com.google.gson.reflect.TypeToken<List<EggRule>>(){}.getType());
    ...
    private static boolean isEggLayingMob(String mobId) {
        List<String> eggMobs = Arrays.asList("minecraft:chicken", "alexsmobs:emu", "naturalist:duck");
    ...
    public static void scanAllMobsAndAddToConfig() {
        ...
        for (String mobId : validAnimals) {
            if (isEggLayingMob(mobId)) {
                EggRule rule = new EggRule(mobId, Arrays.asList("male", "sterile"));
                boolean exists = false;
                for (EggRule existing : data.eggRules) {
                ...
                if (!exists) {
                    data.eggRules.add(rule);
                    GenderMod.LOGGER.info("Auto-added egg rule for: {}", mobId);
                }
            }
        }
    ...
    public static List<String> getAllMobsFromRegistry() {
        ...
        for (var entry : ForgeRegistries.ENTITY_TYPES.getEntries()) {
            allMobs.add(entry.getKey().location().toString());
        }
    ...
    private static boolean isPartOrSegment(String entityId) {
        return entityId.contains("_part") || entityId.contains("_segment") || 
               ...
               entityId.contains("piece") || entityId.contains("tail") || entityId.contains("limb");
    }
    ...
    private static boolean isHostileOrBoss(String entityId) {
        ...
        for (String keyword : hostileKeywords) {
            if (entityId.contains(keyword)) return true;
        }
        return false;
    }
    
    public static boolean isAnimal(EntityType<?> type, String entityId) {
        if (isProjectileOrNonLiving(entityId)) return false;
        if (isPartOrSegment(entityId)) return false;
        if (entityId.equals("minecraft:player") || entityId.equals("minecraft:armor_stand")) return false;
        if (isHostileOrBoss(entityId)) return false;
        
        try {
            if (type.getBaseClass() != null && Animal.class.isAssignableFrom(type.getBaseClass())) return true;
        } catch (Exception e) {}
        try {
            if (type.getBaseClass() != null && WaterAnimal.class.isAssignableFrom(type.getBaseClass())) return true;
        } catch (Exception e) {}
        ...
        try {
            if (type.getBaseClass() != null && AbstractHorse.class.isAssignableFrom(type.getBaseClass())) return true;
        } catch (Exception e) {}
        try {
            if (type.getBaseClass() != null && AbstractGolem.class.isAssignableFrom(type.getBaseClass())) return false;
        } catch (Exception e) {}
    ...
    public static List<String> getGenderOnlyMobs() { return data.settings.genderOnlyMobs; }
    public static List<InteractionRule> getInteractions() { return data.interactions; }
    public static List<EggRule> getEggRules() { return data.eggRules; }
    
    public static boolean isItemBlocked(String mobId, String gender, boolean sterile, Item item) {
        if (data.settings.genderOnlyMobs.contains(mobId)) return false;
        
        ResourceLocation itemId = ForgeRegistries.ITEMS.getKey(item);
        if (itemId == null) return false;
        String itemIdStr = itemId.toString();
        
        for (InteractionRule rule : data.interactions) {
            if (!rule.mobId.equals(mobId)) continue;
            if (!rule.isGenderMatch(gender, sterile)) continue;
            if (rule.isItemMatch(itemIdStr)) return true;
        }
        return false;
    }
    ...
    public static boolean isEggLayingBlocked(String mobId, String gender, boolean sterile) {
        if (data.settings.genderOnlyMobs.contains(mobId)) return false;
        if (data.eggRules == null) return false;
        
        for (EggRule rule : data.eggRules) {
            if (!rule.mobId.equals(mobId)) continue;
            if (rule.isGenderMatch(gender, sterile)) return true;
        }
        return false;
    }
    ...
    public static String[] getRandomGenderWithSterile() {
        ...
        if (roll < male) {
            ...
            gender = male > female ? "male" : (female > male ? "female" : (new Random().nextBoolean() ? "male" : "female"));
            isSterile = true;
        }
    ...
    public static void setAllowMaleMaleBreed(boolean value) { data.settings.allowMaleMaleBreed = value; save(); }
    public static void setAllowFemaleFemaleBreed(boolean value) { data.settings.allowFemaleFemaleBreed = value; save(); }
    public static void setAllowSterileBreed(boolean value) { data.settings.allowSterileBreed = value; save(); }
    public static void setEnableVillagers(boolean value) { data.settings.enableVillagers = value; save(); }
    public static void setKeepVillagerGender(boolean value) { data.settings.keepVillagerGender = value; save(); }
}
